...
public class ScheduleService {
    ...
    public ScheduleResponseDto updateSchedule(Long id, Long userId, String title, String contents) {
        ...
        if (!schedule.getUser().getId().equals(userId)) {//schedule 데이터베이스에 저장된 userId와  내가 집어 넣은 userId가 같은지 확인 처리
            //값을 setter하는건 좋지 않음
            //매서드를 만들어서 해야 함
            ...
            //DB에 있는 해당 Id의 모든 column값 가지고 나옴
    ...
    public void deleteSchedule(Long id, Long userId) {
        ...
        scheduleRepository.delete(schedule);//해당 id에 대한 객체니까 가능!
    }
//예외가 발생하면 throw에 있는 예외가 메세지를 가지고 위로 올라감, 그리고  exceptionhandler에서 맞는 클래스
